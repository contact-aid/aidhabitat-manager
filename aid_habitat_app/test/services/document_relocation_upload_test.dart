...
void main() {
  ...
  for (final confirmed in [true, false]) {
    for (final present in [true, false]) {
      test(
        'queued old-container upload (file present=$present confirmed=$confirmed) preserves intent',
        () async {
            ...
            client: MockClient((request) async {
              ...
              return http.Response(
                jsonEncode({
                  ...
                  'data': {
                    'document': {
                      'remotePath': confirmed ? 'remote-exact' : '',
                      'publicUrl': confirmed
                          ? 'https://synthetic.invalid/doc'
                          : '',
                    },
                  },
                }),
          ...
          expect(result.pushedOperations, present && confirmed ? 1 : 0);
          expect(
            operation['status'],
            !present
                ? 'failed'
          ...
          if (present && !confirmed) {
            ...
            expect(doc['sync_state'], isNot('synced'));
            expect(doc['local_file_path'], path);
            expect(doc['remote_file_path'], isNull);
          }
        },
      );
    }
