...
//   - aid-habitat-manager(-xxx).vercel.app  (API root / legacy React)
//   - aid-habitat-app(-xxx).vercel.app      (Flutter PWA iPad)
export const PROJECT_VERCEL_HOST_PATTERN = /^aid-habitat-(manager|app)(?:-[a-z0-9-]+)?\.vercel\.app$/i;
...
export const readWikiLibraryStore = async () => {
  ...
  // Cf. doc identique côté `server/index.mjs` `readWikiLibraryStore`.
  // En résumé : avant, le filesystem `/tmp/...` éphémère sur Vercel
  // perdait toute modification (ex : 2e description d'un wiki item)
  // au prochain cold start. Maintenant on lit NocoDB en premier où
  // les PUT ont bien persisté, avec fallback filesystem + bundle si
  // NocoDB est inaccessible.
  try {
    ...
    if (Array.isArray(wikiRecords) && wikiRecords.length > 0) {
      ...
      // Best-effort : on persiste aussi côté filesystem pour fallback
      // rapide si NocoDB est temporairement hors service au prochain
      // cycle. Aucune attente — si l'écriture rate, NocoDB reste source.
      try {
  ...
  // Fallback : ancien chemin (filesystem + bundle).
